...
of the TRAPPIST-1 planets.
  ...
  .. |github| replace:: :raw-html:`<a href = "https://github.com/rodluger/planetplanet/blob/master/scripts/mutual_node.py"><i class="fa fa-github" aria-hidden="true"></i></a>`
...
def plot(niter = 10000):
    ...
    for n in tqdm(range(niter)):
        ...
        b = Planet('b', m = m, per = per, inc = inc, r = r, t0 = 0., 
                   Omega = 0, w = w, ecc = ecc, color = 'r')
        ...
        RpRs = N(mu, sig)
        r = RpRs * rstar * RSUN / REARTH
        c = Planet('c', m = m, per = per, inc = inc, r = r, t0 = 0., 
                   Omega = 0, w = w, ecc = ecc, color = 'b')

        # The system
        system = System(star, b, c, nbody = True, 
