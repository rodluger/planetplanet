...
def CustomMap(fwhm = 30, temp = 500):
    ...
    def func(lam, z):
        '''
        The actual function that returns the radiance at a given wavelength 
        and zenith angle.
    ...
    # elliptically symmetric (eyeball) map, whose axis of symmetry rotates as 
    # the planet orbits the star, always pointing towards the substellar point 
    # (or with an offset, if :py:obj:`Lambda` and :py:obj:`Phi` are set). Let's 
    # make this map elliptically symmetric:
...
def view_planet(theta = 0.87, gamma = 4.02):
    ...
    Note that I tuned `theta` and `gamma` to match the hotspot offset specified
    in the `secondary_eclipse` function below.
    
    '''
    
    fig = pl.figure(figsize = (3,3))
    DrawEyeball(fig = fig, radiancemap = CustomMap(), theta = theta, 
                gamma = gamma, nz = 51)
...
def secondary_eclipse():
    ...
    b = Planet('b', m = 1, per = 3, inc = 89.8, r = 5., t0 = -1.5, 
               ...
               phasecurve = True, radiancemap = CustomMap())
