...
def plot():
    ...
    incs = np.array([89.65, 89.67, 89.75, 89.86, 
                     89.680, 89.710, 89.80]) * np.pi / 180
    colors = ['firebrick', 'coral', 'gold', 'mediumseagreen', 
              'turquoise', 'cornflowerblue', 'midnightblue']
    labels = ['b', 'c', 'd', 'e', 'f' , 'g', 'h']
    fig = pl.figure(figsize = (12,5))

    # Plot the star
    x = np.linspace(-rstar, rstar, 1000)
    pl.fill_between(x, -np.sqrt(rstar ** 2 - x ** 2) * AUREARTH, 
    ...
    pl.plot(x, np.sqrt(rstar ** 2 - x ** 2) * AUREARTH, 
...
if __name__ == '__main__':
    fig, _ = plot()
    fig.savefig('orbits.pdf', bbox_inches = 'tight')