...
def _test():
    ...
    eyeball.DrawOrbit(inc = 60., Omega = 0., ecc = 0.5,
                      size = 2, figsize = (6, 6))
    pl.show()
...
if __name__ == '__main__':
    ...
    for i, Lambda, Phi in zip([0, 1], [0, 60], [0, 30]):
        ...
        # Plot the geometry
        fig[i], ax[i] = eyeball.DrawOrbit(inc = inc, Omega = Omega, ecc = ecc, 
                                          w = w, size = 1.75, Lambda = Lambda, 
                                          Phi = Phi, plot_phasecurve = False, 
                                          label_phases = True, 
                                          rasterize = True)
