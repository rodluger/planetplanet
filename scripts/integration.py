...
Plots a diagram showing how the occultation integration scheme works. 
  ...
  .. |github| replace:: :raw-html:`<a href = "https://github.com/rodluger/planetplanet/blob/master/scripts/integration.py"><i class="fa fa-github" aria-hidden="true"></i></a>`
...
def plot():
    ...
    for za in np.linspace(0, np.pi, nl + 2)[1:-1]:
        ...
        inds = np.where((x >= x1) & (x <= x2))
        ax.plot(x[inds], y[inds], 'b-', zorder = 999, lw = 2)
        ax.plot(x[inds], -y[inds], '-', zorder = 999, lw = 2, 
                color = '#aaaaee')
    ...
    # Integration bounds
    for x in (x1, x2):
        ax.plot([x, x], [yo + np.sqrt(ro ** 2 - (x - xo) ** 2), 
                -np.sqrt(r ** 2 - x ** 2)], '-', lw = 2, color = '#aaaaff')
        ax.plot([x, x], [yo - np.sqrt(ro ** 2 - (x - xo) ** 2), 
                np.sqrt(r ** 2 - x ** 2)], 'b-', lw = 2)
    ...
    ax.annotate(r'$A_2$', xy = (-0.54, 0.64), xycoords = 'data', ha = 'center', 
                va = 'center', color = 'b', fontsize = 14)
    ax.annotate(r'$A_3$', xy = (-0.4, 0.87), xycoords = 'data', ha = 'center', 
                va = 'center', color = 'b', fontsize = 14, xytext = (-20, 40), 
                textcoords = 'offset points', 
                arrowprops = dict(arrowstyle = '-', color = 'b'))
    ax.annotate(r'$x_n$', xy = (-0.70711, 0.70711), xycoords = 'data', 
                ha = 'center', va = 'center', color = 'r', 
                xytext = (-14, 8), textcoords = 'offset points', fontsize = 14)
    ax.annotate(r'$x_{n+1}$', xy = (-0.334832, -0.309744), xycoords = 'data', 
                ha = 'center', va = 'center', color = 'r', 
                xytext = (20, -8), textcoords = 'offset points', fontsize = 14)

    ax.annotate(r'$f_0$', xy = (-0.54, -0.44), xycoords = 'data', 
                ha = 'center', va = 'center', color = 'b', fontsize = 8)
    ax.annotate(r'$f_1$', xy = (-0.54, 0.42), xycoords = 'data', 
                ha = 'center', va = 'center', color = 'b', fontsize = 8)
    ax.annotate(r'$f_2$', xy = (-0.54, 0.76), xycoords = 'data', 
                ha = 'center', va = 'center', color = 'b', fontsize = 8)
    ax.annotate(r'$f_3$', xy = (-0.54, 0.9), xycoords = 'data', 
                ha = 'center', va = 'center', color = 'b', fontsize = 8)

    ax.annotate(r'$\mathcal{O}$', xy = (-1.54, 1.41), xycoords = 'data', 
                ha = 'center', va = 'center', color = 'k', fontsize = 20)
    ax.annotate(r'$\mathcal{P}$', xy = (0.76, -0.85), xycoords = 'data', 
                ha = 'center', va = 'center', color = 'k', fontsize = 20)
