...
def test_mutual(tol = 1e-10):
    ...
    assert np.abs(values[1] - truths[1]) / truths[1] < tol, \
           "Incorrect flux: %.10e != %.10e." % (values[1], truths[1])
    assert np.abs(values[2] - truths[2]) / truths[2] < tol, \
           "Incorrect average flux: %.10e != %.10e." % (values[2], truths[2])
    assert np.abs(values[3] - truths[3]) / truths[3] < tol, \
           "Incorrect average flux: %.10e != %.10e." % (values[3], truths[3])

def test_compare_methods(tol = 1e-6):
    '''
    Compare the different optimization methods for limb-darkened transits, 
    including the `batman` algorithm.
    ...
    # Check!
    assert (flux1min - flux2min) / flux1min < tol, \
           "Incorrect flux: %.10e != %.10e" % (flux1min, flux2min)
    assert (flux1min - flux3min) / flux1min < tol, \
           "Incorrect flux: %.10e != %.10e" % (flux1min, flux3min)
    assert (flux1mean - flux2mean) / flux1mean < tol, \
           "Incorrect average flux: %.10e != %.10e" % (flux1mean, flux2mean)
    assert (flux1mean - flux3mean) / flux1mean < tol, \
           "Incorrect average flux: %.10e != %.10e" % (flux1mean, flux3mean)

def test_occultation(tol = 1e-10):
    '''
    Test a planet-planet occultation of an airless body with phase 
    curves on and using the N-body solver.
    
    '''
    
    # Instantiate the Trappist-1 system
    system = Trappist1(sample = False, phasecurve = True, 
                       airless = True, nbody = True)
    ...
    assert (flux2min - truths[1]) / truths[1] < tol, \
           "Incorrect flux: %.10e != %.10e" % (flux2min, truths[1])
    assert (flux1mean - truths[2]) / truths[2] < tol, \
           "Incorrect average flux: %.10e != %.10e" % (flux1mean, truths[2])
    assert (flux2mean - truths[3]) / truths[3] < tol, \
           "Incorrect average flux: %.10e != %.10e" % (flux2mean, truths[3])
