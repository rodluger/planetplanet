...
class System(object):
    ...
    def scatter_plot(self, tstart, tend, dt = 0.001):
                    ...
                    for occ1 in occultors:
                        for occ2 in occultors:
                            if self.bodies[occ1].occultor[i] & 2 ** occ2:
                                axp.plot(self.bodies[occ1].x[i], 
                                         self.bodies[occ1].z[i], 'x',
                                         color = self.colors[occ2], 
        ...
        for j, body in enumerate(self.bodies):
            ...
            axl1.plot(x, y, 'o', color = self.colors[j], ms = 6, alpha = 1, 
                      markeredgecolor = 'none')
        ...
        for j, impact in enumerate([0.6, 0.8, 1.]):
            ...
            axl3.plot(0.675, -0.75 * j, 'o', color = 'k', ms = 8, 
                      alpha = alpha, markeredgecolor = 'none')
            axl3.annotate('%.1f' % impact, xy = (0.775, -0.75 * j), 
                          xycoords = 'data', ha = 'left', va = 'center', 
                          color = 'k')
