...
def GetAngles(x, y, z, vx, vy, vz, Lambda = 0., Phi = 0.):
  '''
  Computes the eyeball angles :math:`\\theta` and :math:`\gamma` given the Cartesian
  orbital elements and the hotspot offset angles :math:`\Lambda` and :math:`\Phi`.
  ...
  :param float Lambda: The longitudinal hotspot offset in radians. Default `0`
  :param float Phi: The latitudinal hotspot offset in radians. Default `0`
  :returns: :math:`\\theta` and :math:`\gamma`, the eyeball phase angles, in radians
...
def DrawOrbit(radiancemap = RadiativeEquilibriumMap(), inc = 70., Omega = 0., ecc = 0., w = 0., Phi = 0., Lambda = 0., nphases = 20, size = 1, 
  ...
  for i in inds:
    ...
    theta, gamma = GetAngles(b.x[i], b.y[i], b.z[i], b.vx[i], b.vy[i], b.vz[i], Lambda = Lambda, Phi = Phi)
